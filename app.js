...
// ── Defense Calendar Invite ──
function saveDefenseDetails() {
  state.defenseDetails = {
    time: document.getElementById('defense-time').value || '10:00',
    duration: parseInt(document.getElementById('defense-duration').value) || 60,
    room: (document.getElementById('defense-room').value || '').trim()
  };
  saveState();
  history.replaceState(null, '', '#' + encodeState());
}

function _defenseDateTime() {
  const def = state.defenseDetails || {};
  const dateStr = state.dateOverrides && state.dateOverrides['defense_deadline'];
  if (!dateStr) return null;
  const d = parseDate(dateStr);
  const parts = (def.time || '10:00').split(':');
  d.setHours(parseInt(parts[0]) || 10, parseInt(parts[1]) || 0, 0, 0);
  return d;
}

function downloadDefenseICS() {
  saveDefenseDetails();
  const def = state.defenseDetails || {};
  const start = _defenseDateTime();
  if (!start) return;
  const end = new Date(start.getTime() + (def.duration || 60) * 60000);
  const room = def.room || '';
  const title = state.studyTitle || 'Thesis';
  const name = state.name || 'Student';
  const stamp = toICSDate(new Date());
  const uid = `defense-${state.bpStart}-${Date.now()}@dexlab-planner`;
  ...
  ics.push('BEGIN:VALARM', 'TRIGGER:-P1D', 'ACTION:DISPLAY', 'DESCRIPTION:Thesis defense tomorrow', 'END:VALARM');
  ...
  const a = document.createElement('a');
  a.href = url;
  a.download = 'thesis-defense.ics';
  document.body.appendChild(a);
  a.click();
  document.body.removeChild(a);
  URL.revokeObjectURL(url);
  showToast('Defense invite downloaded — import into Outlook or Apple Calendar.');
}

function openDefenseGCal() {
  saveDefenseDetails();
  const def = state.defenseDetails || {};
  const start = _defenseDateTime();
  if (!start) return;
  const end = new Date(start.getTime() + (def.duration || 60) * 60000);
  const room = def.room || '';
  const title = encodeURIComponent(`Thesis Defense — ${state.name || 'Student'}`);
  const details = encodeURIComponent(`Thesis defense for "${state.studyTitle || 'Thesis'}" by ${state.name || 'Student'}.`);
  const location = encodeURIComponent(room);
  const fmt = d => d.toISOString().replace(/[-:]/g, '').replace(/\.\d{3}/, '');
  const guests = state.supervisorEmail ? `&add=${encodeURIComponent(state.supervisorEmail)}` : '';
  const url = `https://calendar.google.com/calendar/render?action=TEMPLATE&text=${title}&dates=${fmt(start)}/${fmt(end)}&details=${details}&location=${location}${guests}`;
  ...
  sectionOrder.filter(key => sections[key]).forEach(sectionKey => {
    ...
    items.forEach(m => {
      ...
      if (!isFixedMs && _editingDateId === m.id) {
        ...
        const isDefense = m.id === 'defense_deadline';
        const editBtnClass = isDefense && !m.overridden ? 'action-btn action-btn-accent' : 'action-btn';
        const editBtnLabel = isDefense && !m.overridden ? 'Set defense date' : 'Edit date';
        actions += `<button onclick="event.stopPropagation();startDateOverride('${m.id}')" class="${editBtnClass}" title="Edit date">${editBtnLabel}</button>`;
      ...
      // Defense scheduling panel
      let defensePanel = '';
      if (m.id === 'defense_deadline' && m.overridden) {
        const def = state.defenseDetails || {};
        defensePanel = `
          <div class="defense-panel" onclick="event.stopPropagation()">
            <div class="defense-panel-title">Schedule your defense</div>
            <div class="defense-fields">
              <div class="defense-field">
                <label for="defense-time">Time</label>
                <input type="time" id="defense-time" value="${def.time || '10:00'}" onchange="saveDefenseDetails()">
              </div>
              <div class="defense-field">
                <label for="defense-duration">Duration (min)</label>
                <input type="number" id="defense-duration" value="${def.duration || 60}" min="15" max="180" onchange="saveDefenseDetails()">
              </div>
              <div class="defense-field">
                <label for="defense-room">Room</label>
                <input type="text" id="defense-room" value="${escapeHTML(def.room || '')}" placeholder="e.g. C1.008" onchange="saveDefenseDetails()">
              </div>
            </div>
            <div style="margin-top:8px;display:flex;gap:8px;flex-wrap:wrap;">
              <button class="btn btn-sm btn-primary" onclick="event.stopPropagation();downloadDefenseICS()">Download .ics invite</button>
              <button class="btn btn-sm btn-ghost" onclick="event.stopPropagation();openDefenseGCal()">Google Calendar</button>
            </div>
